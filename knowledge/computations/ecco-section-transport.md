...
executor:
  resource: skills/transport-analysis/scripts/ecco_section_transport.py
  receipt: [run_id, code_sha256, data, bound_parameters, resolved_section, results, mutation_evidence, caveats]
attester:
  resource: skills/transport-analysis/scripts/section_transport_check.py
...
sources:
  ...
  - id: mht-26n-recipe
    resource: knowledge/podaac/recipes/ecco-mht-26n.md
...
`knowledge/podaac/computations/ecco-section-transport.md`, and the
...
executor `skills/transport-analysis/scripts/ecco_section_transport.py`
is byte for byte the file that was under the bundle, so its sha256
8e14b704b9610d12 and every receipt that names it stand. The reference
run `section-transport` reads the ECCO 2010 fixture cache, which this
...
[^mht-26n-recipe]: knowledge/podaac/recipes/ecco-mht-26n.md, the recorded 1.098 PW global-circle anchor
