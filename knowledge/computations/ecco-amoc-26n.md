...
computation: skills/transport-analysis/scripts/ecco_amoc_26n.py
executor:
  resource: skills/transport-analysis/scripts/ecco_amoc_26n.py
...
attester:
  resource: skills/compare-obs/scripts/rapid_confrontation_check.py
...
sources:
  ...
  - id: basin-codes
    resource: skills/transport-analysis/scripts/llc90_basin_codes.py
  ...
  - id: recipe
    resource: knowledge/podaac/recipes/ecco-rapid-amoc-26n.md
...
`knowledge/podaac/computations/ecco-amoc-26n.md`, and the executor and
the attester it names now ship in this package under
`skills/transport-analysis/scripts/`. The golden
`verification/section_transport.py` names every script it does. The
executor `skills/transport-analysis/scripts/ecco_amoc_26n.py` changed
only where it resolves a path, so its sha256 moved from 5e6ea8d5ecb2493b
to 17338a1381c4ec20 and no number in it did. This computation reads the
ECCO science record and the hashed basin mask, which is committed here
but the record is not, so it has no entry in
verification/reference_runs.yaml and was not re-run. The committed
exhibit-amoc-26n-record.json no longer attests, because the executor now
reaches the mask under knowledge/references/retrieval; the maintainer
regenerates it where the record is present. The signature block is the
one the maintainer left; the concept sits at `status: draft` with no
verified event added, and the maintainer re-signs it after merge.
...
[^basin-codes]: skills/transport-analysis/scripts/llc90_basin_codes.py and the hashed masks under knowledge/references/retrieval/
