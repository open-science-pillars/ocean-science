...
executor:
  resource: skills/sea-level-analysis/scripts/ecco_regional_sea_level.py
...
sources:
  ...
  - id: gotcha-ssh-ib
    resource: knowledge/podaac/gotchas/ecco-ssh-ib-variants.md
    title: "Bundle gotcha: ECCO SSH inverse-barometer variants (steward-verified 2026-07-06)"
  - id: fields-ssh
    resource: knowledge/podaac/fields/ecco-v4r4/ssh.md
    title: "Bundle fields concept: sea surface height (stable)"
  - id: fields-obp
    resource: knowledge/podaac/fields/ecco-v4r4/obp.md
...
`knowledge/podaac/computations/ecco-regional-sea-level.md`, and the
...
executor `skills/sea-level-analysis/scripts/ecco_regional_sea_level.py`
changed only where it resolves a path, so its sha256 moved from
e8382a304bc7c486 to a65155566bd4d18d and no number in it did. The
reference run `sea-level` reads the ECCO 2010 fixture cache, which this
environment does not hold, so it was not re-run here; the golden runs it
where the cache is present. The two committed partition exhibits no
longer attest, because they were produced by the executor before its
import of the trend method moved; the maintainer regenerates them where
the cache is present. The signature block is the one the maintainer
...
[^gotcha-ssh-ib]: knowledge/podaac/gotchas/ecco-ssh-ib-variants.md, one SSH variant, named, never mixed
