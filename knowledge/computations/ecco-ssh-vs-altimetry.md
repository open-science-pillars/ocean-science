...
computation: skills/compare-obs/scripts/ecco_ssh_vs_altimetry.py
executor:
  resource: skills/compare-obs/scripts/ecco_ssh_vs_altimetry.py
...
attester:
  resource: skills/compare-obs/scripts/altimetry_confrontation_check.py
...
sources:
  ...
  - id: record-note
    resource: nasa-daac-knowledge/docs/nasa-ssh-record.md
  ...
  - id: large-scale-domain
    resource: knowledge/podaac/validity-domains/ecco-large-scale-statistics.md
...
`knowledge/podaac/computations/ecco-ssh-vs-altimetry.md`, and the
...
`skills/compare-obs/scripts/ecco_ssh_vs_altimetry.py` changed only where
it resolves a path, so its sha256 moved from 9bd868932c0c5779 to
b0b9d3d68afb61ca and no number in it did. This comparison reads a
stamped NASA-SSH tree, which this package carries a manifest for but
does not commit, so it was not re-run. The committed
exhibit-ssh-vs-altimetry-record.json no longer attests, because the
executor now reaches the partition, the scoring and the trend method
across skills; the maintainer regenerates it where the tree is present.
