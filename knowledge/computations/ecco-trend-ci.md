...
executor:
  resource: skills/ecco/scripts/ecco_trend_ci.py
  receipt: [run_id, code_sha256, data, bound_parameters, series, intermediates, results]
attester:
  resource: skills/ecco/scripts/trend_ci_check.py
calibration: skills/ecco/scripts/ecco_trend_ci_calibration.py
calibration_report: knowledge/references/retrieval/trend-ci-coverage.json
...
sources:
  ...
  - id: calibration-report
    resource: knowledge/references/retrieval/trend-ci-coverage.json
  ...
  - id: joint-gotcha
    resource: knowledge/podaac/gotchas/ecco-trend-deseasonalize-jointly.md
  ...
  - id: naked-trend
    resource: knowledge/podaac/gotchas/ecco-trend-without-effective-n.md
...
`knowledge/podaac/computations/ecco-trend-ci.md`, and the executor and
the attester it names now ship in this package under
`skills/ecco/scripts/`. The golden `verification/trend_ci.py` names
every script it does. The executor
`skills/ecco/scripts/ecco_trend_ci.py` is byte for byte the file that
was under the bundle, so its sha256 4a03a106e15f2fc4 and every receipt
that names it stand. The reference run `trend-ci` of
verification/reference_runs.yaml was re-run at the new paths over the
committed steric exhibit and reproduced the committed trend exhibit to
1e-9 relative on every number, attested PASS; the coverage calibration
holds its own assertions. The signature block is the one the maintainer
...
[^calibration-report]: knowledge/references/retrieval/trend-ci-coverage.json, the seeded Monte Carlo report
...
[^naked-trend]: knowledge/podaac/gotchas/ecco-trend-without-effective-n.md, the trap this closes
[^sea-level-partition]: computations/ecco-regional-sea-level.md, three trends, three embedded blocks
[^joint-gotcha]: knowledge/podaac/gotchas/ecco-trend-deseasonalize-jointly.md, the sawtooth in the calendar-month means of time
