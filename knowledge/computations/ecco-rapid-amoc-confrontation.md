...
computation: skills/compare-obs/scripts/ecco_rapid_amoc_confrontation.py
executor:
  resource: skills/compare-obs/scripts/ecco_rapid_amoc_confrontation.py
...
sources:
  ...
  - id: rapid-record
    resource: nasa-daac-knowledge/docs/rapid-26n-record.md
  ...
  - id: doctrine
    resource: knowledge/podaac/conventions/consistency-versus-confrontation.md
...
`knowledge/podaac/computations/ecco-rapid-amoc-confrontation.md`, and
the executor and the attester it names now ship in this package under
`skills/compare-obs/scripts/`. The golden `verification/compare_obs.py`
names every script it does. The executor
`skills/compare-obs/scripts/ecco_rapid_amoc_confrontation.py` changed
only where it resolves a path, so its sha256 moved from 3cb4a9efc034d462
to 2291ce86f37df637 and no number in it did. This confrontation reads a
stamped RAPID tree, which this package carries a manifest for but does
not commit, so it was not re-run. The committed
exhibit-rapid-amoc-26n-confrontation.json no longer attests, because the
executor now reaches the overturning and the trend method across skills;
the maintainer regenerates it where the tree is present. The signature
block is the one the maintainer left; the concept sits at `status:
draft` with no verified event added, and the maintainer re-signs it
after merge.
