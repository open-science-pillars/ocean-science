...
the run procedure of the ocean-budget skill and returns PASS (exit 0) only when ALL
of the following hold, otherwise FAIL (exit 1) naming the failing field:
  ...
  2. code_sha256 equals the sha256 of the sanctioned computation file
     (ecco_heat_budget.py beside this file), so a rewritten or
...
DEFAULT_COMPUTATION = (Path(__file__).resolve().parent / "ecco_heat_budget.py")
